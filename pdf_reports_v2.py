...
def _hm(minutes: Optional[Union[int, float]]) -> str:
    if minutes is None:
        return "-"
...
def _observation(day: Optional[dict]) -> str:
    if not day:
        return "-"
    if day.get("absence"):
        return "Falta"
...
def _punches(day: Optional[dict]) -> str:
    if not day:
        return "-"
    values = []
...
def _calendar_rows(month: str, days: list[dict]) -> list[tuple[date, Optional[dict]]]:
...
def _register_employee_pdf_route() -> None:
    ...
    def atlas_employee_pdf(employee_id: int, month: Optional[str] = None):
